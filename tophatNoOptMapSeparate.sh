...
for sample in ${samples[*]}; do
	...
	mkdir $tophatOutputDir/${sample}_1
	bsubcommand="bsub $scriptDir/tophatNoOptMapSeparateJobArgList.sh ${sample}_1 $lfilelist $scriptDir"
	echo $bsubcommand
	eval $bsubcommand
	
	
	mkdir $tophatOutputDir/${sample}_2
	bsubcommand="bsub $scriptDir/tophatNoOptMapSeparateJobArgList.sh ${sample}_2 $rfilelist $scriptDir"
	echo $bsubcommand
	eval $bsubcommand
